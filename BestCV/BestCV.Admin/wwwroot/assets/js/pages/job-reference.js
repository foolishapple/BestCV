...
$(document).ready(function () {
    ...
    $("#form-submit-job-reference").on("submit", function (e) {
        e.preventDefault();
        validateJobSuitable();
    })
...
async function editItem(id) {
    ...
    if (id > 0) {
        ...
        if (updatingObj != null && updatingObj != undefined) {
            ...
            swal.fire(
                'việc làm liên quan',
                'không thể cập nhật việc làm liên quan, hãy kiểm tra lại thông tin.',
                'error'
            );
        }
    ...
    $("#jobReferenceModalTitle").text(id > 0 ? "Cập nhật việc làm liên quan" : "Thêm mới việc làm liên quan");
    $("#loading").removeClass("show");
    $("#jobReferenceModal").modal("show");
}

async function getItemById(id) {
    await $.ajax({
        url: systemConfig.defaultAPIURL + "api/job-reference/detail/" + id,
        type: "GET",
...
function loadData() {
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/job-reference/list-aggregates",
        type: "GET",
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa việc làm liên quan',
        html: 'Bạn có chắc chắn muốn xóa việc làm liên quan <b>' + item.jobName + '</b>?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $("#loading").addClass("show");
            $.ajax({
                url: systemConfig.defaultAPIURL + "api/job-reference/delete?id=" + id,
                type: "DELETE",
                success: function (response) {
                    $("#loading").removeClass("show");
                    if (response.isSucceeded) {
                        Swal.fire(
                            'Xóa việc làm liên quan',
                            'Việc làm liên quan <b>' + item.jobName + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        reGenTable();
                    }
                        ...
                        if (response.status == 400) {
                            if (response.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Việc làm liên quan <p class="swal__admin__subtitle"> Xóa không thành công </p>',
                                    contentError,
                                    'warning'
                                );
                            } else {
                ...
                error: function (e) {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Xóa việc làm liên qua',
                            'Xóa việc làm liên qua không thành công, <br> vui lòng thử lại sau!',
                            'error'
                        );
                    }
...
function validateJobSuitable() {
    ...
    if (errorList.length > 0) {
        ...
        Swal.fire(
            'Việc làm liên quan' + swalSubTitle,
            contentError,
            'warning'
        );
    } else {
...
function submit() {
    ...
    swal.fire({
        title: actionName + " việc làm liên qua",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " việc làm liên qua <b>" + $("#searchJobId option:selected").text() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingId > 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/job-reference/update",
                    type: "PUT",
                    contentType: "application/json",
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Cập nhật việc làm liên qua',
                                'Việc làm liên qua <b>' + $("#searchJobId option:selected").text() + '</b> đã được cập nhật thành công.',
                                'success'
                            ).then((result) => {
                                $("#jobReferenceModal").modal("hide");
                                reGenTable();
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Việc làm liên qua <p class="swal__admin__subtitle"> Cập nhật không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
                    ...
                    error: function (e) {
                        ...
                        if (e.status === 401) {
                            ...
                            swal.fire(
                                'Việc làm liên quap',
                                'không thể cập nhật việc làm liên qua, hãy kiểm tra lại thông tin.',
                                'error'
                            );
                        }
            ...
            if (updatingId == 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/job-reference/add",
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify(obj),
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Thêm mới việc làm liên qua',
                                'Việc làm liên qua <b>' + $("#searchJobId option:selected").text() + '</b> đã được thêm mới thành công.',
                                'success'
                            ).then(function () {
                                $("#jobReferenceModal").modal("hide");
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Việc làm liên qua <p class="swal__admin__subtitle"> Thêm mới không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
                    ...
                    error: function (e) {
                        ...
                        if (e.status === 401) {
                            ...
                            Swal.fire(
                                'Quản lý việc làm liên qua',
                                'Không thể thêm mới việc làm liên qua, hãy kiểm tra lại thông tin.',
                                'error'
                            );
                        }
                    }
...
function loadDataJobSelect() {
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/job-reference/list-job-selected",
        type: "GET",
        contentType: "application/json",
        success: function (response) {
            //dataSource = response.resources;
