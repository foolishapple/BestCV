...
$("#menuHomepageName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    ...
    if (id > 0) {
        ...
        $("#selectMenuType").val(DEFAULT_VALUE_MENU_DASHBOARD_CANDIDATE).trigger("change");
...
function loadMenuType() {
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/menu-type/list-menu-type",
        type: "GET",
        contentType: "application/json",
        success: function (response) {
...
function loadTable() {
    ...
    dataSource.forEach(function (item, index) {
        ...
        rowContent += '<td data-sort="' + moment(item.createdTime, "YYYY-MM-DD HH:mm:ss").format("YYYY-MM-DD HH:mm:ss") + '">' + moment(item.createdTime, "YYYY-MM-DD HH:mm:ss").format("DD/MM/YYYY HH:mm:ss") + '</td>';
...
function validateJobSkill() {
    ...
    if ($("#menuHomepageName").val().length == 0) {
        errorList.push("Tên không được bỏ trống.");
    } else if ($("#menuHomepageName").val().length > 255) {
        errorList.push("Tên không được dài quá 255 ký tự.");
    }

    if ($("#menuHomepageLink").val().length == 0) {
        errorList.push("Link không được bỏ trống.");
    }
...
function submit() {
    var obj = {
        ...
        menuTypeId: ($("#selectMenuType").val() != '' ? $("#selectMenuType").val() : ""),
...
$.fn.dataTable.ext.search.push(
    function (settings, data, dataIndex) {
        ...
        var dataSearch = $("#searchMenuType").val() || "";
