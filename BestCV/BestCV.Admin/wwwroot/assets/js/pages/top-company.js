...
$("#topCompanyOrderSort").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
function loadData() {
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/top-company/list-by-orderSort",
        type: "GET",
...
function loadTable() {
    ...
    dataSource.forEach(function (item, index) {
        ...
        rowContent += "<td>" + "<span style='display : none;'>" + (item.topCompanyId != null ? item.topCompanyId : "") + "</span>" + (item.topCompanyName != null ? item.topCompanyName : "") + "</td>";
...
function loadDataCompany() {
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#searchTopCompanyName").select2({
                allowClear: true,
                placeholder: ""
            });

            $("#topCompanyName").select2({
                allowClear: true,
                placeholder: ""
            });
        ...
        swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (obj) {
                    try {
                        let result;
                        result = await httpService.postAsync("api/top-company/change-order-slide", obj);
...
function submit() {
    ...
    if (updatingId > 0 && updatingObj && updatingObj.id !== undefined) {
        var obj = {
            ...
            topCompanyId: updatingObj.topCompanyId,
            orderSort: ($("#topCompanyOrderSort").val() != '' ? $("#topCompanyOrderSort").val() : ""),
...
$.fn.dataTable.ext.search.push(
    function (settings, data, dataIndex) {
        ...
        var dataSearch = $("#searchTopCompanyName").val() || "";
