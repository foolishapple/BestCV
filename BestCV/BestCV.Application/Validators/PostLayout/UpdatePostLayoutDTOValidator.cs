...
{
    public class UpdatePostLayoutDTOValidator : AbstractValidator<UpdatePostLayoutDTO>
    {
        public UpdatePostLayoutDTOValidator()
        {
            RuleFor(x => x.Id)
                .NotNull().WithMessage("Mã layout không được bỏ trống.")
                .GreaterThan(0).WithMessage("Mã layout phải lớn hơn 0.");
            RuleFor(x => x.Name)
