...
async function loadData() {
    try {
        let res = await httpService.postAsync(`api/candidate-save-job/get-list-job`);
        if (res.isSucceeded) {
            if (res.status == 200) {
...
async function deleteNoti(id) {
    ...
    swal.fire({
        title: 'Xóa thông báo',
        html: 'Bạn có chắc chắn muốn xóa công việc <br><b>' + item.jobName + '</b>?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/candidate-save-job/delete/${id}`);
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Xóa thông báo',
                            'Công việc đã lưu đã được xóa thành công',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Xóa thông báo',
                        'Xóa công việc đã lưu không thành công, vui lòng thử lại!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
