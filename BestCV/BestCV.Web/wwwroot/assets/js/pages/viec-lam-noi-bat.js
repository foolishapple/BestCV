...
function loadDataJobFeatureFeature() {
    var obj = {
    ...
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/top-feature-job/searching-feature-job",
        type: "POST",
            ...
            if (dataSourceJob.length > 0) {
                dataSourceJob.forEach(function (item) {
                   //convert ve dang slug
                    let slugJob = stringToSlug(item.topFeatureJobName);
                        ...
                        <div class="row align-items-center justify-content-between">
                            ...
                            <div class="col-9">
                                <div class="d-flex flex-column">
                                    <a href="/chi-tiet-cong-viec/${slugJob + "-" + item.topFeatureJobId}" class="pxp-jobs-card-3-title mt-3 mt-sm-0 ${dataRedTitle != undefined ? "text-danger" : ""}">${dataNew != undefined && (moment() - moment(item.jobCreatedTime)) < dayToMilisecond ? newTag : ""}${item.topFeatureJobName}</a>
                                    <div>
                            ...
                            <div class="col-1">
                                <a class="rbt-round-btn btn-save-job" title="Bookmark" href="#!" data-job-id='${item.topFeatureJobId}' data-job-name='${item.topFeatureJobName}'>
