...
// * author: Thoại Anh
// * created: 08.08.2023
// * Description: Trang thông báo ứng viên
...
function initTablenoti() {
    table = $("#table_candidate_notification").DataTable({
        ...
        ordering: false,
        ajax: {
            url: `${systemConfig.defaultAPIURL}api/candidate-notification/list-candidate-notification`,
            type: "POST",
        ...
        drawCallback: function () {
            $('#table_candidate_notification tfoot').html("");
...
async function makeAsRead(event) {
    ...
    let res = await httpService.putAsync(`api/candidate-notification/MakeAsRead/${notificationId}`);
    if (res.isSucceeded) {
        if (res.status == 200) {
...
//        //drawCallback: function () {
//        //    //$('#table_candidate_notification tfoot').html("");
//        //    //$("#table_candidate_notification thead:nth-child(1) tr").clone(true).appendTo("#table_candidate_notification tfoot");
//        //}
...
async function getItemById(id) {
    await $.ajax({
        url: systemConfig.defaultAPIURL + "api/candidate-notification/detail/" + id,
        type: "GET",
...
$(".candidate-notification").on("click", ".page-item", function (e) {
    e.preventDefault();
...
//    table.column(1).search($("#table_candidate_notification thead:nth-child(2) tr th:nth-child(2) input").val());
//    table.column(3).search($("#table_candidate_notification thead:nth-child(2) tr th:nth-child(4) input").val());
//    table.column(4).search($("#table_candidate_notification thead:nth-child(2) tr th:nth-child(5) input").val());
//    table.draw();
//}
...
async function deleteNoti(id) {
    ...
    swal.fire({
        ...
        if (swalResult.isConfirmed) {
            ...
            try {
                let deleteResult = await httpService.deleteAsync(`api/candidate-notification/delete/${id}`);
...
async function loadDataAndCountNotifications() {
    $(".noti_body").html("");
    try {
        let res = await httpService.getAsync(`api/candidate-notification/listRecent`);
...
async function loadUnreadTotal() {
    ...
    try {
        let res = await httpService.getAsync(`api/candidate-notification/GetUnreadTotal`);
        if (res.isSucceeded && res.status === 200) {
